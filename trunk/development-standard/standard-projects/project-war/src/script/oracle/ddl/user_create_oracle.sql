...
/* 회원_기본설정 */
DROP TABLE T_USER_BASIC_SETTING_LOG;

CREATE TABLE T_USER_BASIC_SETTING_LOG
(
  C_ID        NUMBER                            NOT NULL,
  C_PARENTID  NUMBER                            NOT NULL,
  C_POSITION  NUMBER                            NOT NULL,
...
COMMENT ON TABLE T_USER_BASIC_SETTING_LOG IS '회원_비밀번호보안수준 트리거 로그';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_ID IS '노드 아이디';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_PARENTID IS '부모 노드 아이디';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_POSITION IS '노드 포지션';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_LEFT IS '노드 좌측 끝 포인트';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_RIGHT IS '노드 우측 끝 포인트';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_LEVEL IS '노드 DEPTH ';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_TITLE IS '노드 명';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_TYPE IS '노드 타입';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_METHOD IS '노드 변경 행위';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_STATE IS '노드 상태값 ( 이전인지. 이후인지)';
COMMENT ON COLUMN T_USER_BASIC_SETTING_LOG.C_DATE IS '노드 변경 시';
...
CREATE OR REPLACE TRIGGER "TRIGGER_USER_BASIC_SETTING"
BEFORE DELETE OR INSERT OR UPDATE
ON T_USER_BASIC_SETTING
REFERENCING NEW AS NEW OLD AS OLD
FOR EACH ROW
...
BEGIN
   tmpVar := 0;
    IF UPDATING  THEN    
        insert into T_USER_BASIC_SETTING_LOG 
        values (:old.C_ID,:old.C_PARENTID,:old.C_POSITION,:old.C_LEFT,:old.C_RIGHT,:old.C_LEVEL,:old.C_TITLE,:old.C_TYPE,'update','변경이전데이터',sysdate);        
        insert into T_USER_PROH_WORD_LOG 
        values (:new.C_ID,:new.C_PARENTID,:new.C_POSITION,:new.C_LEFT,:new.C_RIGHT,:new.C_LEVEL,:new.C_TITLE,:new.C_TYPE,'update','변경이후데이터',sysdate);   
     END IF;
   ...
   EXCEPTION
     WHEN OTHERS THEN
       -- Consider logging the error and then re-raise
       RAISE;
END TRIGGER_USER_BASIC_SETTING;
/* 류강하 끝 */
...
CREATE TABLE T_USER_GRADE_MANAGE (
	...
	C_RIGHT NUMBER NOT NULL, /* 노드우측끝포인트 */
	C_LEVEL NUMBER NOT NULL, /* 노드레벨 */
	C_TITLE VARCHAR2(4000) NOT NULL, /* 회원등급명 */
	C_TYPE VARCHAR2(100), /* 노드타입 */
...
CREATE TABLE T_USER_MENU_BY_GRADE (
	...
	C_RIGHT NUMBER NOT NULL, /* 노드우측끝포인트 */
	C_LEVEL NUMBER NOT NULL, /* 노드레벨 */
	C_TITLE VARCHAR2(4000) NOT NULL, /* 메뉴명 */
	C_TYPE VARCHAR2(100), /* 노드타입 */
