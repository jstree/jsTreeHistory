...
COMMENT ON COLUMN T_USER_INFO.C_PWD_FIND_ANSWER IS '비밀번호찾기답변';
...
COMMENT ON TABLE T_USER_LOGIN_STATE_LOG IS '회원_로그인현황 트리거 로그';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_ID IS '노드 아이디';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_PARENTID IS '부모 노드 아이디';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_POSITION IS '노드 포지션';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_LEFT IS '노드 좌측 끝 포인트';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_RIGHT IS '노드 우측 끝 포인트';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_LEVEL IS '노드 DEPTH ';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_TITLE IS '노드 명';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_TYPE IS '노드 타입';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_METHOD IS '노드 변경 행위';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_STATE IS '노드 상태값 ( 이전인지. 이후인지)';
COMMENT ON COLUMN T_USER_LOGIN_STATE_LOG.C_DATE IS '노드 변경 시';
...
COMMENT ON TABLE T_USER_JOIN_STATE_LOG IS '회원_가입상태 트리거 로그';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_ID IS '노드 아이디';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_PARENTID IS '부모 노드 아이디';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_POSITION IS '노드 포지션';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_LEFT IS '노드 좌측 끝 포인트';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_RIGHT IS '노드 우측 끝 포인트';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_LEVEL IS '노드 DEPTH ';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_TITLE IS '노드 명';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_TYPE IS '노드 타입';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_METHOD IS '노드 변경 행위';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_STATE IS '노드 상태값 ( 이전인지. 이후인지)';
COMMENT ON COLUMN T_USER_JOIN_STATE_LOG.C_DATE IS '노드 변경 시';
...
COMMENT ON COLUMN T_USER_NOTE_BY_USER.C_TITLE IS '쪽지제목';
...
CREATE TABLE T_USER_POINT_SETTING (
	...
	C_TITLE VARCHAR2(4000) NOT NULL, /* 노드명 */
	C_TYPE VARCHAR2(100), /* 노드타입 */
	C_POINT_USE_FL CHAR(1) DEFAULT 0, /* 포인트사용여부 */
	C_DOWNLOAD_PROH_FL CHAR(1) DEFAULT 0, /* 다운로드금지여부 */
	C_ARTICLE_READ_PROH_FL CHAR(1) DEFAULT 0, /* 금열람금지여부 */
	C_POINT_EXPIRY_DATE NUMBER, /* 포인트유효기간 */
	C_USER_JOIN_POINT NUMBER, /* 회원가입포인트 */
	C_LOGIN_POINT NUMBER, /* 로그인포인트 */
	C_WRITING_POINT NUMBER, /* 글쓰기포인트 */
	C_ANSWER_POINT NUMBER, /* 댓글포인트 */
	C_UPLOAD_POINT NUMBER, /* 업로드포인트 */
	C_DOWNLOAD_POINT NUMBER, /* 다운로드포인트 */
	C_RECMNDER_POINT NUMBER, /* 추천인포인트 */
	C_DOWNLOAD_DEDU_POINT NUMBER, /* 다운로드차감포인트 */
	C_ARTICLE_READ_DEDU_POINT NUMBER /* 글열람차감포인트 */
);
