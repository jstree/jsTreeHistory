...
<!--<![endif]-->
	<head data-placeholder-focus="false" data-placeholder-live="false">
		...
		function fn_main_headPageMove(url){
			window.location.href = "index.do?link="+url;
			return;
		}
		</script>
										...
										<c:otherwise>
											...
											<c:otherwise>
												...
												<li class="sub-nav-item${subCssValue }">
													<a href="javascript:fn_main_headPageMove('${result.url }');" target="_self" class="${subCssValue }">&raquo; &nbsp;${result.c_title }</a>
												</li>
					...
					<div id="secondary-navigation" class="clearfix">
						...
						<c:forEach items="${menuList }" var = "result" varStatus="status">
							...
							<c:when test = "${result.c_position == '0' }" >
									<a href="javascript:fn_main_headPageMove('${result.url }');" target="_self" class="sub-nav-item first active">${result.c_title }</a>
							</c:when>
							<c:otherwise>
								<a href="javascript:fn_main_headPageMove('${result.url }');" target="_self" class="sub-nav-item">${result.c_title }</a>
							</c:otherwise>
							</c:choose>
							<c:set var="count" value="${count } +1" />
							</c:otherwise>
